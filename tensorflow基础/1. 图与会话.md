...
启动会话的第一步是创建一个session对象。会话提供在图中执行操作的一些列方法。一般的模式是，建立会话，此事会生成一张空图，在会话中添加图，然后执行。建立会话可以有两种方式，一种是`tf.Session()`，通常使用这种方法创建会话；另一种是`tf.InteractiveSession()`，这种方式更多的用在iPython notebook等交互式Python环境中。
...
有时候我们需要建构多个图，在不同的图中定义不同的计算，根据我们的需要在session中调用不同的图。这时候需要用到图构建方法。
...
*在使用notebook时，由于notebook的一些特性，必须指定默认图，否则程序会报错。为了书写规范，推荐尽量书写完整默认图。*
