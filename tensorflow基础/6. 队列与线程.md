...
1. 与单队列意义，使用`tf.train.QueueRunner()`创建一个队列管理器。一个队列管理器管理一个队列。队列管理器中需要传入将要执行的操作以及执行的线程数量。例如：
...
2. `tf.train.add_queue_runner()`向图的队列管理器集合中添加一个队列管理器。一个图可以有多个队列管理器。例如：
...
3. `tf.train.start_queue_runners()`在会话中开启所有的队列管理器。这时候，我们可以将线程协调器传递给它，用于完成线程执行。例如：
